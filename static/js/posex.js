...
function init_3d(ui) {
    ...
    const default_body = add_body('default', 0, 0, 0);
    ...
    renderer.domElement.addEventListener('pointermove', e => {
        e.preventDefault();
        m.x = (e.offsetX / canvas.width) * 2 - 1;
        m.y = (1 - e.offsetY / canvas.height) * 2 - 1;
        rc.setFromCamera(m, camera);
        const intersects = rc.intersectObjects(touchable_objects);

        if (intersects.length > 0) {
            const object = intersects[0].object;
            notation.textContent = object.name;
            notation.style.left = `${e.clientX}px`;
            notation.style.top = `${e.clientY - 20}px`;
